...
| Paired usefulness comparison | complete | `skill-agent candidate-usefulness --baseline-run-id --treatment-run-id` compares pinned no-temporary-skill control and temporary-skill treatment runs while rejecting contaminated baselines and preserving read-only boundaries |
| Exact plan-digest admission approval | complete | `admit-candidate --dry-run` emits a stable plan digest and requires matching non-expired append-only `approve_review` notes before readiness or write-evidence preparation |
| Permission/dependency diff packet | complete | `admit-candidate --dry-run` reports semantic permission, tool, and dependency declaration diffs and blocks dependency declarations without an exact realization contract |
| Exact dependency realization evidence | complete | Dependency declarations can include name/version/sha256 realization evidence; the preview distinguishes missing realization from unsupported dependency install |
| Counterfactual skill receipt | complete | `skill-agent skill-receipt` aggregates origin, utility, containment, compatibility, approval, and reversibility proof categories without mutating ledgers, registry, snapshots, staging, or durable skills |
| Negative evidence report | complete | `skill-agent negative-evidence` surfaces reject/defer/block/repair resolution history and blocked/repair/duplicate candidate evidence without rewriting historical ledgers |
| Tamper-evident local checkpoints | complete | `skill-agent evidence-checkpoint` appends or verifies a local hash-chain over core `runs/` evidence without rewriting run logs, ledgers, durable skills, registry, or governor behavior |
| Non-steering evidence governor | complete | `skill-agent evidence-governor` recommends ask/test_more/deny/defer from existing proof surfaces without granting approval, installing, promoting, widening permissions, routing, or steering execution |
| Managed shadow activation plan | complete | `skill-agent shadow-activation-plan` reports a content-addressed managed store path, profile generation, activation pointer, rollback target, and shadow plan digest without creating files or switching profiles |
