...
def test_v1_release_eval_suite_passes_without_durable_skill_mutation(
    ...
    assert dimensions["v1_release"]["total"] == 10
    assert dimensions["release_gate"]["passed"] == 10
    assert dimensions["stable_readiness"]["total"] == 3
    assert dimensions["stable_routing_disabled"]["total"] == 3
